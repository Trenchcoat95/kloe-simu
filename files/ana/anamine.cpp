...
void FillHisto(TChain& t1, TChain& t2, TH1D& h1, TH1D& h2, int nEv, const char* templ1, const char* templ2, const char* varexp, TCut& cut)
{
  ...
  int long n2 = t2.Draw(TString::Format("%s>>%s", varexp, name2.Data()).Data(),cut,"E0", nEv, 0);
...
void processChain(TChain& t)
{
  ...
  TH1F *htempa = (TH1F*)gPad->GetPrimitive("hframe");
  htempa->GetYaxis()->SetTitle("y[m]");
  htempa->GetXaxis()->SetTitle("x[m]");
  t.Draw("yv_true/1000:xv_true/1000","","same",10000,0);
  TBox b(kloe_center[0]*0.001-emcalo_barrel_mod_halfl*0.001,kloe_center[1]*0.001-emcalo_ext_radius*0.001,kloe_center[0]*0.001+emcalo_barrel_mod_halfl*0.001,kloe_center[1]*0.001+emcalo_ext_radius*0.001);
  b.SetFillStyle(0);
  b.Draw();
  ...
  TH1F *htempb = (TH1F*)gPad->GetPrimitive("hframe");
  htempb->GetYaxis()->SetTitle("z[m]");
  htempb->GetXaxis()->SetTitle("y[m]");
  t.Draw("yv_true/1000:zv_true/1000","","same",10000,0);
  TEllipse elint(kloe_center[2]*0.001,kloe_center[1]*0.001,emcalo_int_radius*0.001,emcalo_int_radius*0.001);
  TEllipse elext(kloe_center[2]*0.001,kloe_center[1]*0.001,emcalo_ext_radius*0.001,emcalo_ext_radius*0.001);
  ...
  htemp2->GetYaxis()->SetTitle("y[m]");
  htemp2->GetXaxis()->SetTitle("z[m]");
  t.SetMarkerColor(kRed);
  t.SetMarkerStyle(1);
...
void anamine()
{
  ...
  c1.SaveAs(path + "ptrueVSprecoQ.png");
   ...
   /////////////////////////////////ptrueXVSprecoX Quality Cut
  
  TH2D h_pQx("h_pQx","",200,-5,5,200,-5,5);
  h_pQx.GetXaxis()->SetTitle("p_{#mu}^{true} [GeV/c]");
  h_pQx.GetYaxis()->SetTitle("p_{#mu}^{reco} [GeV/c]");
  tNom.Draw("pxmu_reco/1E3:pxmu_true/1E3>>h_pQx",fiducial + CC + muonOK + lowE + QualityCut,"");
  h_pQx.Draw("colz");

  c1.SaveAs(path + "result.pdf","pdf");
  c1.SaveAs(path + "ptruexVSprecoxQ.png");
  
  /////////////////////////////////ptrueVSpreco no Quality Cut
  
  TH2D h_pnoQ("h_pnoQ","",200,0,20,200,0,20);
  h_pnoQ.GetXaxis()->SetTitle("p_{#mu}^{true} [GeV/c]");
  h_pnoQ.GetYaxis()->SetTitle("p_{#mu}^{reco} [GeV/c]");
  tNom.Draw("pmu_reco/1E3:pmu_true/1E3>>h_pnoQ",fiducial + CC + muonOK + lowE,"");
  h_pnoQ.Draw("colz");

  c1.SaveAs(path + "result.pdf","pdf");
  c1.SaveAs(path + "ptrueVSpreconoQ.png");
  ...
  TH2D h_reldpl_chi2("h_reldpl_chi2","",100,0,20000,100,-1,1);
  ...
  c1.SaveAs(path + "result.pdf","pdf");
  c1.SaveAs(path + "pl_chi2_lnBig.png");
 ...
 */ 
  ...
  for(int i = 0; i < nn; i++)
  {
    ...
    TCut cut = lowE + fiducial + CC + muonOK + QualityCut; 
